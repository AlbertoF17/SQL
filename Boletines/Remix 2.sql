...
SELECT ship.CompanyName, count(distinct(orddet.productID)) FROM orderdetails AS orddet JOIN orders AS ord ON (orddet.OrderID = ord.OrderID) JOIN shippers AS ship ON (ord.ShipVia = ship.ShipperID) GROUP BY ship.CompanyName;
